...
@pytest.mark.asyncio
async def test_written_audit_record_cannot_be_updated() -> None:
    """Append-only is enforced by the ORM, not just by convention (P2-20j)."""
    engine = create_async_engine("sqlite+aiosqlite:///:memory:")
    session_factory = async_sessionmaker(engine, expire_on_commit=False)
    try:
        async with engine.begin() as connection:
        ...
        async with session_factory.begin() as session:
            await log_in(session, action=AuditAction.LOGIN, actor_username="admin")

        async with session_factory() as session:
            row = (await session.execute(select(AuditLog))).scalar_one()
            row.actor_username = "someone-else"
...
@pytest.mark.asyncio
async def test_written_audit_record_cannot_be_deleted() -> None:
    engine = create_async_engine("sqlite+aiosqlite:///:memory:")
    session_factory = async_sessionmaker(engine, expire_on_commit=False)
    try:
        async with engine.begin() as connection:
